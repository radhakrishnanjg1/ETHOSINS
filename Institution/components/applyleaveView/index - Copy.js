...
(function () {
    var view = app.applyleaveView = kendo.observable();
    var validator;
    var applyleaveViewModel = kendo.observable({
        onShow: function () {
            if (!app.utils.checkinternetconnection()) {
                return app.navigation.navigateoffline("applyleaveView");
            }
    ...
    view.set('applyleaveViewModel', applyleaveViewModel);
}());
...
function fun_applyleaveencashment() {
...
function fun_applyleave_savevalidation() {
    ...
    //var isdcrdateExists = $('#hdndcrdate').val(); 
    var totaleldays = $('#hdntotalel').val();
    ...
    if (currentyear != fromyear || currentyear != toyear) {
        ...
        if (daysbal >= 0) {
            ...
            if (leavetype == "867") {
                ...
                if ($('#hdntotalel').val() != "0") {
                    if (parseInt($('#hdntotalel').val()) < (-1) * daysbal) {
                        // EBO.EL_Days = hfTotalEL.Value;
                        //$('#txteldays').val();
                        //EBO.LOP_Days = ((-1) * DaysBal - parseInt($('#hdntotalel').val())).ToString(); 
                        $('#txtlopdays').val((-1) * daysbal - parseInt($('#hdntotalel').val()));
        ...
        else if (leavetype == "2234" || 2242 == "869") { //Ml and Pl to allow
            fun_applyleave_request();
        }
    }
}
...
function fun_applyleave_request() {
    ...
    fun_db_APP_Insert_Ethos_Leave_Master(parseInt($('#hdnEmployee_ID').val()), parseInt($('#ddlleavetype').val()), parseInt($('#txtopeningbalance').val()),
         ...
         parseInt($('#txtdays').val()), parseInt($('#hdnReporting_Employee_ID').val()), $('#txtreason').val(),
         $('#txtaddress').val(), $('#txtmobilenumber').val(), 1,
         parseInt($('#hdnLogin_ID').val()), parseInt($('#hdnLogin_ID').val()), parseInt($('#txtlopdays').val()),
         parseInt($('#txteldays').val()));
}
...
function fun_db_APP_Get_Employee_Leave_Details(leavetype, Employee_ID) {
    ...
    datasource.fetch(function () {
        ...
        if (data[0].Leave_Type_ID > 0) {
            ...
            if (leavetype == 869) {
                //int numberofdays = Convert.ToInt32(dv[0]["Closing_Bal"]);
                //txtEligible.Text = (((numberofdays - 30) / 15) * 15).ToString().Trim();
