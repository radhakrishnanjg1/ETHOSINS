...
(function () {
    var view = app.cancelleaveView = kendo.observable();
    var cancelleaveViewModel = kendo.observable({
        onShow: function () {
            if (!app.utils.checkinternetconnection()) {
                return app.navigation.navigateoffline("cancelleaveView");
            }
            app.navigation.logincheck();
            app.utils.loading(true);
              fun_db_APP_Get_Ethos_Leave_Cancel(parseInt($('#hdnEmployee_ID').val()));
            //fun_db_APP_Get_Ethos_Leave_Cancel(979);
            $('#hdn_leavecancel_ethos_leave_master_id').val('0');
        },  
    }); 
    view.set('cancelleaveViewModel', cancelleaveViewModel);
}());
