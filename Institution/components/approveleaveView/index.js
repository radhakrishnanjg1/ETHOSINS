...
(function () {
    var view = app.approveleaveView = kendo.observable();
    var validator;
    var approveleaveViewModel = kendo.observable({
        onShow: function () {
            if (!app.utils.checkinternetconnection()) {
                return app.navigation.navigateoffline("approveleaveView");
            }
            app.navigation.logincheck();
            app.utils.loading(true);
            fun_db_APP_Get_Ethos_Leave_Approve(parseInt($('#hdnEmployee_ID').val()));
            $('#hdnethos_leave_master_id').val('0');
            //fun_db_APP_Get_Ethos_Leave_Approve(2478); 
        },  
    });

    view.set('approveleaveViewModel', approveleaveViewModel);
}());
...
function fun_leave_reject(e) {
    var data = e.button.data();
    $('#hdnethos_leave_master_id').val(data.ethos_leave_master_id);  
...
function fun_leave_reject_submit() {
    ...
    if (remarks != "") {
        var ethos_leave_master_id = parseInt($('#hdnethos_leave_master_id').val());
