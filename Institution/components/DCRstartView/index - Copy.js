    ...
    var DCRstartViewModel = kendo.observable({
        onShow: function () {
            ...
            var render_dcr_master_id = function (tx, rs) {
                // alert("hdndcr_master_id: " + rs.rows.item(0).dcr_master_id + "|activity_id:" + rs.rows.item(0).activity_id);
            ...
            if (parseInt($("#hdndcr_master_id").val()) == 1 && (parseInt($("#hdndcr_master_id").val()) == 235)) {
                alert("hdndcr_master_id: " + parseInt($("#hdndcr_master_id").val()) + "|activity_id:" + parseInt($("#hdndcr_master_id").val()));
                var render_dcr_ins_master = function (tx, rs) {
                    //alert("hdndcr_ins_master_id: " + rs.rows.item(0).dcr_ins_master_id);
                    $("#hdndcr_ins_master_id").val(rs.rows.item(0).dcr_ins_master_id);
                }
                app.select_count_dcr_ins_master_bydcr_master_id(render_dcr_ins_master, hdndcr_master_id);
                var render_dcr_unlisted_ins_master = function (tx, rs) {
                    // alert("hdndcr_unlisted_ins_master_id: " + rs.rows.item(0).hdndcr_unlisted_ins_master_id);
                    $("#hdndcr_unlisted_ins_master_id").val(rs.rows.item(0).dcr_unlisted_ins_master_id);
                }
               ...
               || parseInt($("#hdnactivity_id").val()) == 256 || parseInt($("#hdnactivity_id").val()) == 258
                ...
                alert("hdndcr_master_id: " + parseInt($("#hdndcr_master_id").val()) + "|activity_id:" + parseInt($("#hdnactivity_id").val()));
